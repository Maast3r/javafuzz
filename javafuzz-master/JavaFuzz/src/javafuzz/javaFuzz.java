...
public class javaFuzz {
	...
	public static void main(String[] args) {
		...
		while ((c = g.getopt()) != -1) {
			...
			case 'p':
				...
				if (values.length != 2) {
					...
					if (values[0].equals("int")) {
						pp = (Object) Integer.parseInt(values[1]);
					} else if (values[0].equals("float")) {
						pp = (Object) Float.parseFloat(values[1]);
					} else if (values[0].equals("double")) {
						pp = (Object) Double.parseDouble(values[1]);
					} else if (values[0].equals("short")) {
						pp = (Object) Short.parseShort(values[1]);
					} else if (values[0].equals("long")) {
						pp = (Object) Long.parseLong(values[1]);
			...
			case 'g':
				...
				if (arg.equals("a")) {
					...
					for (int findAllTypes = 0; findAllTypes < getReplacements.length; findAllTypes++) {
						String[] replacementValues = getReplacements[findAllTypes]
								.split("=");
						if (replacementValues.length != 2) {
							usage();
							System.exit(0);
						} else if ((replacementValues.length == 2)
								&& (replacementValues[1].equals("{A}"))) {
							try {
								findThisClass.add(replacementValues[0]);
								if (Class.forName(replacementValues[0])
										.isInterface())
									replaceItWithThisClass.add(findSubclass(
											replacementValues[0],
											"FindSubs.txt"));
								else if (Modifier.isAbstract((Class
										.forName(replacementValues[0]))
										.getModifiers()))
									replaceItWithThisClass.add(findSubclass(
											replacementValues[0],
											"FindSubs.txt"));
							...
							findThisClass.add(replacementValues[0]);
							replaceItWithThisClass.add(replacementValues[1]);
						}
					}
					ANCA = 1;
				}
			...
			case 'r':
				...
				arg = g.getOptarg();
				try {
					Recursion = Integer.parseInt(arg);
				} catch (Exception e) {
					usage();
					System.exit(0);
				}
			...
			case 'l':
				...
				arg = g.getOptarg();
				try {
					StringSize = Integer.parseInt(arg);
				} catch (Exception e) {
					usage();
					System.exit(0);
				}
		...
		if (Start.equals("")) {
			IamBigString = BigString("A", StringSize);
		} else {
			IamBigString = BigString(Start, StringSize);
		}
		...
		cmin = '\u0000';
		cmax = '\uffff';
		// Limits : boolean true/false - this one doesnt make much sense but
		// anyways
		bomin = false;
		bomax = true;
		// Limits : string
		...
		// Multi-dimensional arrays -- monkey business at the moment will do it
		// more genericly soon
		...
		// Limits : int minimum value of -2,147,483,648 and a maximum value of
		// 2,147,483,647
		ai = new int[ArraySize];// {imax,imin}
		aii = new int[ArraySize][ArraySize];// {imax,imin}
		// Limits : long minimum value of -9,223,372,036,854,775,808 and a
			...
			if (!cc.equals("")) {
				try {
					...
					if (vv == 1 && (!InterfacesAndAbstracts.equals(""))) {
						System.out.println("\n************************");
						System.out
								.println("The following classes may not have been instantiated");
						System.out
...
//		className = "com.Evolution.logic.Game";
		...
		try {
			...
			System.out.println("no classname " + className);
			e.printStackTrace();
		}
		if (pp == null) {
			try {
	...
	public static void methodSlap(Constructor cs, Class cls, Object[] args,
			int hilo, int v) {
		try {
				...
				if (checkFO == 1) {
					...
					try {
						...
						for (int checkIgnore = 0; checkIgnore < nim.length; checkIgnore++) {
							if (!allMethods[a].getName().equals(
									nim[checkIgnore])) {
								Justincase = 1;
							}
						}
		...
		try {
			...
			for (int k = 0; k < cls.length; k++) {
				...
				if (current.equals("int")) {
					if (max) {
						list[k] = (imax + ExceedInt);
					} else {
						list[k] = imin - (ExceedInt);
					}
					...
					if (max) {
						list[k] = fmax + ExceedFloat;
					} else {
						list[k] = fmin - (ExceedFloat);
					}
					...
					if (max) {
						list[k] = smax + ExceedShort;
					} else {
						list[k] = smin - (ExceedShort);
					}
					...
					if (max) {
						list[k] = dmax + ExceedDouble;
					} else {
						list[k] = dmin - (ExceedDouble);
					}
					...
					if (max) {
						list[k] = lmax + ExceedLong;
					} else {
						list[k] = lmin - (ExceedLong);
					}
					...
					list[k] = astt;
				} else if (current.indexOf("[L") != -1
						&& current.indexOf(";") != -1) {
					list[k] = Array.newInstance(Class.forName(current
							.substring(2, current.length() - 1)), ArraySize);
				}
				// Construct - Uknown Object
					...
					try {
						...
						// Replace abstract with a constructor that can
						// implement it
						if (ANCA == 1) {

							for (int lookThroughAndCheck = 0; lookThroughAndCheck < findThisClass
									.size(); lookThroughAndCheck++) {
								if (findThisClass.get(lookThroughAndCheck)
										.equals(clsa.getName())) {
									clsa = Class.forName(""
											+ replaceItWithThisClass
													.get(lookThroughAndCheck));
								}
							}
						...
						if (AutoBreak == 1) {
							if (clsa.isInterface()) {
								clsa = Class.forName(findSubclass(
										clsa.getName(), "FindSubs.txt"));
							}
							if (Modifier.isAbstract(clsa.getModifiers())) {
								clsa = Class.forName(findSupers(clsa.getName(),
										"FindSubs.txt"));
							}
						}
						...
						if (clsa.isInterface()) {
							...
							if (InterfacesAndAbstracts.indexOf(clsa.getName()
									+ " is an Interface") == -1) {
	...
	static void recursiveAttack(String FileName, int v) throws Exception {
		...
		while (in.available() != 0) {
			try {
				javaFuzz.summarize(in.readLine(), v);
			} catch (Exception e) {
			}
		}
		in.close();
	...
	private static void usage() {
		System.out
				.println("\n= =============================================== =");
		System.out
				.println("= JavaFuzzer - Classes Fuzzing (Reflection Based) =");
		System.out
				.println("= =============================================== =\n");
